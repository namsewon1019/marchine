...
char *drinkname = (char *)malloc(sizeof(char) * 100);
char *drinknamechoise = (char *)malloc(sizeof(char) * 100);
char *adddrinkname = (char *)malloc(sizeof(char) * 100);
...
int showmenu(linkedList_h *H) {
   ...
   insertNode("레몬에이드",1800,H);
   insertNode("갈배사이다",2000,H);
   insertNode("콜라",1800,H);
   insertNode("사이다",1800,H);
   insertNode("래쓰비",1000,H);
...
int main() {
	...
	while(1) {
		...
		scanf("%d", &number);	
		switch(number) {
			case 1 : showmenu(H); break; //메뉴출력 (구현 : O) 
			case 2 : buy(H); break; // 음료수 구매 (구현 : O) 
			case 3 : insert(); break; // 음료수 추가 (구현 : X) 
